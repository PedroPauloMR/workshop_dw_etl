...
def log(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(
            f"Iniciando a função '{func.__name__}' às {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}"
        )
        try:
            ...
            logger.success(
                f"A função '{func.__name__}' completou com sucesso em {duration:.2f} segundos"
            )
            return result
